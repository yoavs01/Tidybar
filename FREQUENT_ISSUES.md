...
Check here before [filing a bug](https://github.com/yoavs01/Tidybar/issues/new/choose). Your problem may already be known with a workaround available.

> [!IMPORTANT]
> **macOS 27 (Golden Gate):** use the tracking issue **[#687](https://github.com/yoavs01/Tidybar/issues/687)** for general support, status, and preview builds. Do not open a new issue for that.
...
General macOS 27 support, incomplete behavior on Golden Gate, and preview builds are tracked in **[#687](https://github.com/yoavs01/Tidybar/issues/687)**. Comment there instead of opening a new issue.
...
Related reports: [#607](https://github.com/yoavs01/Tidybar/issues/607), [#707](https://github.com/yoavs01/Tidybar/issues/707), [#675](https://github.com/yoavs01/Tidybar/issues/675), [#605](https://github.com/yoavs01/Tidybar/issues/605).
...
Related reports: [#702](https://github.com/yoavs01/Tidybar/issues/702), [#717](https://github.com/yoavs01/Tidybar/issues/717), [#718](https://github.com/yoavs01/Tidybar/issues/718).
...
|---------|----------------|
| Icon not listed by name in Layout settings | [#709](https://github.com/yoavs01/Tidybar/issues/709) |
| Icon moves back to Hidden / Always Hidden after a few minutes | [#651](https://github.com/yoavs01/Tidybar/issues/651), [#575](https://github.com/yoavs01/Tidybar/issues/575) |
| Icon won't stay where you place it (regression in 2.0.0-beta.13) | [#643](https://github.com/yoavs01/Tidybar/issues/643) |
| Dragging in Layout settings has no lasting effect | [#372](https://github.com/yoavs01/Tidybar/issues/372), [#709](https://github.com/yoavs01/Tidybar/issues/709) |
| Clicking the icon while hidden makes the Thaw icon follow the cursor | [#332](https://github.com/yoavs01/Tidybar/issues/332) |
| Duplicate Little Snitch Agent after wake from sleep | [#641](https://github.com/yoavs01/Tidybar/issues/641) |

**What works today**

- **2.0.0-beta.14+** includes improved matching for Control Center–hosted items ([#643](https://github.com/yoavs01/Tidybar/issues/643)). The beta 13 regression ("will not stick") should not recur on current builds.
...
2. If the icon vanishes when Thaw launches, **quit Thaw**, **⌘ Command + drag** the Little Snitch icon to the far right of the menu bar, then relaunch Thaw ([#709](https://github.com/yoavs01/Tidybar/issues/709)).
3. Clear Thaw's cache and reset Accessibility permission, then re-grant when prompted ([#643](https://github.com/yoavs01/Tidybar/issues/643)):

   ```sh
   rm -rf ~/Library/Caches/com.yoavsror.tidybar
   tccutil reset Accessibility com.yoavsror.tidybar
   ```

4. If Little Snitch **Network Monitor meters** keep returning to Always Hidden, try placing them from Little Snitch's own preferences rather than only through Thaw ([#575](https://github.com/yoavs01/Tidybar/issues/575)).
...
- The icon may never appear under the name "Little Snitch" in Layout settings even when visible in the bar ([#709](https://github.com/yoavs01/Tidybar/issues/709)).
...
CodexBar issues are **upstream**: certain CodexBar versions wrote state into macOS Control Center preferences that caused Thaw (and other menu bar managers) to misplace the icon into Hidden or Always Hidden ([#605](https://github.com/yoavs01/Tidybar/issues/605)).

**Affected CodexBar versions (from issue reports)**

| Version | Status |
|---------|--------|
| **0.27.0** | Reported broken ([#605](https://github.com/yoavs01/Tidybar/issues/605) comment) |
| **0.28.0** | Worked in maintainer testing |
| **0.29.0** | Known bad: corrupts Control Center files ([#605](https://github.com/yoavs01/Tidybar/issues/605)) |
| **0.29.1** | Fixed for some users |
| **0.30.1+** | Upstream fix shipped ([#605](https://github.com/yoavs01/Tidybar/issues/605), [CodexBar v0.30.1](https://github.com/steipete/CodexBar/releases/tag/v0.30.1), [steipete/CodexBar#1122](https://github.com/steipete/CodexBar/pull/1122)) |
...
2. If the menu bar still behaves oddly after upgrading, reset Control Center preferences ([#605](https://github.com/yoavs01/Tidybar/issues/605)):
...
Corrupted Control Center state from CodexBar 0.29.x can also cause **Little Snitch**, **Timemator**, and other Control Center–hosted items to drift ([#643](https://github.com/yoavs01/Tidybar/issues/643) comments). If multiple apps misbehave at once, fix CodexBar first.
...
Related reports: [#591](https://github.com/yoavs01/Tidybar/issues/591), [#685](https://github.com/yoavs01/Tidybar/issues/685), [#641](https://github.com/yoavs01/Tidybar/issues/641), [#708](https://github.com/yoavs01/Tidybar/issues/708).
...
3. If a system app crashes when spacing changes (for example Spotlight), treat it as an upstream macOS issue ([#720](https://github.com/yoavs01/Tidybar/issues/720)).

Related reports: [#664](https://github.com/yoavs01/Tidybar/issues/664).
...
Without the permission, Thaw draws each item as its owning app's icon in the Thaw Bar, the layout bars, and the search panel. Older builds showed an error on those surfaces instead, which is what [#628](https://github.com/yoavs01/Tidybar/issues/628) reported.

Grant permissions under **Settings → Advanced → Permissions**. If macOS keeps re-prompting after reboot despite approval ([#683](https://github.com/yoavs01/Tidybar/issues/683)), remove Thaw from **System Settings → Privacy & Security → Screen Recording**, then add it again.
...
Related report: [#610](https://github.com/yoavs01/Tidybar/issues/610).

## Flickering, refreshing, or "dancing" icons

Frequent redraws are often caused by the **host app** updating its status item (for example Shazam or Amphetamine) rather than by Thaw. Mission Control and space switches can also trigger full layout storms ([#718](https://github.com/yoavs01/Tidybar/issues/718)).
...
Related reports: [#678](https://github.com/yoavs01/Tidybar/issues/678), [#649](https://github.com/yoavs01/Tidybar/issues/649).

## Live Activities and iPhone mirroring

Live Activities mirrored from an iPhone (Screen Continuity) can interfere with layout operations ([#722](https://github.com/yoavs01/Tidybar/issues/722), [#556](https://github.com/yoavs01/Tidybar/issues/556)). Gaps or empty spaces in the menu bar during mirroring are a known limitation.

**Workaround:** Pause iPhone mirroring or wait until the Live Activity ends before editing layout.

## High CPU or memory usage

Sustained high CPU or RAM after long uptime is not expected ([#680](https://github.com/yoavs01/Tidybar/issues/680), [#599](https://github.com/yoavs01/Tidybar/issues/599)).
...
2. Search [open and closed issues](https://github.com/yoavs01/Tidybar/issues?q=is%3Aissue) for duplicates.
...
5. Use the [bug report template](https://github.com/yoavs01/Tidybar/issues/new/choose). Reports without enough detail to reproduce may be closed until more information is provided.
